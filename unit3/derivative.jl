...
function plot_derivative(f, df; min_x = -2, max_x = 2, min_y = -2, max_y = 2)
    ...
    xs,ys = meshgrid(range(min_x, stop=max_x, length=11), range(min_y,stop=max_y,length=11))

    p1 = surface(X, Y, f, fillalpha=0.8, legend=false, camera=(45,40), 
                 xtickfontsize=14, ytickfontsize=14, xguidefontsize=16, yguidefontsize=16, zguidefontsize=16)
