...
"""
function plot_function_sample(basis,n=99,d=5;min_x=0,max_x=5, color1=:blue, color2=:red)
    xs = range(min_x,max_x,1000)
    v = randn(d,1)
    p = plot(xs, map(x -> f(x,v,basis), xs), legend=false, linewidth = 2, color = color1, 
             xtickfontsize=14, ytickfontsize=14, xguidefontsize=16, yguidefontsize=16)
    for i = 1:n
        v = randn(d,1)
        plot!(xs, map(x -> f(x,v,basis), xs), legend=false, linewidth = 0.3, color = color2)
    end
    xlabel!(L"x")
    ylabel!(L"f(x)")
