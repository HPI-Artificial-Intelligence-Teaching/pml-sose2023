# Plots for fast Bayesian regression with linear basis function and predictive distributions
#
...
"""
    factorized_Bayesian_linear_regressio(train_data; σ=0.1, τ=0.5, ϕ = x -> map(j -> polynomial_basis(x, j), 1:2))

Computes the posterior mean and diagonal covariance from the 1D Gaussian update equations coming out of the sum-product algorithm.
"""
function factorized_Bayesian_linear_regression(
    train_data;
    σ = 0.1,
    τ = 0.5,
    ϕ = x -> map(j -> polynomial_basis(x, j), 1:2),
)
    n = length(ϕ(train_data.x[1]))
    μ = zeros(n)
    σ2 = ones(n) * τ^2

    for i in eachindex(train_data.y)
        x = ϕ(train_data.x[i])
        total_variance = σ^2 + sum(x.*x .* σ2)
        Δ = train_data.y[i] - sum(μ.*x)
        for j = 1:n
            μ[j] = μ[j] + Δ * σ2[j] * x[j] / total_variance
            σ2[j] *= (1 - x[j]*x[j] * σ2[j] / total_variance)
        end
        println("μ = ", μ)
        println("σ2 = ", σ2)
    end

    return(μ,Diagonal(σ2))
end
...
function plot_Bayesian_fit(
    ...
    if (factorized)
        (μ,S) = factorized_Bayesian_linear_regression(train_data; σ=σ, τ=τ, ϕ=ϕ)
    else
...
plot_Bayesian_fit(
    ...
    τ = 0.1,
    color = :blue,
    ϕ = x -> map(j -> gauss_basis(x, j / 2, σ = 0.15), 0:20),
)
plot_Bayesian_fit(
    train_data,
    τ = 0.1,
...
plot_Bayesian_fit(
    ...
    τ = 0.1,
    color = :blue,
    ϕ = x -> map(j -> gauss_basis(x, j / 5, σ = 0.15), 0:20),
)
plot_Bayesian_fit(
    train_data,
    τ = 0.1,
...
display(p)
savefig("~/Downloads/gauss_fit2.svg")
...
plot_Bayesian_fit(
    ...
    τ = 0.1,
    color = :blue,
    ϕ = x -> map(j -> gauss_basis(x, j / 20, σ = 0.35), 0:20),
)
plot_Bayesian_fit(
    train_data,
    τ = 0.1,
...
display(p)
savefig("~/Downloads/gauss_fit3.svg")
