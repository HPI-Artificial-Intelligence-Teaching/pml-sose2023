...
πs = 0:0.01:1.0
p = plot(legend=false, xtickfontsize=14, ytickfontsize=14, xguidefontsize=16, yguidefontsize=16)
...
n = Normal()
xs = -4:0.1:4.0
p = plot(xtickfontsize=14, ytickfontsize=14, xguidefontsize=16, yguidefontsize=16, legendfontsize=16)
plot!(xs,map(x -> cdf(n,sqrt(pi/8)*x),xs), label=L"\Phi\left(\sqrt{\frac{\pi}{8}}x\right)", linewidth=3)
plot!(xs,map(x -> sigmoid(x),xs), label=L"{\mathrm{logit}}(x)", linewidth=3, linecolor=:red)
xlabel!(L"x")
display(p)