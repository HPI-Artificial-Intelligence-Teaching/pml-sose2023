...
"""
function fit_polynomial(train_data; degree=2, min_degree=1, color=:blue, threed=false)
...
p = plot(train_data.x, train_data.y, seriestype=:scatter, legend=false, color = :orange, xtickfontsize=14, ytickfontsize=14)
xlabel!(L"x")
ylabel!(L"y")
display(p)
...
p = plot(train_data.x, train_data.y, seriestype=:scatter, legend=false, color = :orange, xtickfontsize=14, ytickfontsize=14, ztickfontsize=14)
xlabel!(L"x")
ylabel!(L"y")
...
p = surface(x, y, (x, y) -> pdf(mv,[x, y]), color=:blues, fillalpha=0.1, legend=false, xtickfontsize=14, ytickfontsize=14, ztickfontsize=14)
...
p = surface(w1, w2, (w1, w2) -> likelihood([w1,w2],train_X,train_data.y)[1], color=:reds, fillalpha=0.1, legend=false, xtickfontsize=14, ytickfontsize=14, ztickfontsize=14)
...
w1_coarse = range(3, stop = 5, length = 30)
w2_coarse = range(-5, stop = -3, length = 30)
p = surface(w1, w2, (w1, w2) -> likelihood([w1,w2],train_X,train_data.y)[1] * pdf(mv,[w1, w2]), color=:greens, fillalpha=0.1, legend=false, xtickfontsize=14, ytickfontsize=14, ztickfontsize=14)
wireframe!(w1_coarse, w2_coarse, (w1, w2) -> likelihood([w1,w2],train_X,train_data.y)[1] * pdf(mv,[w1, w2]))
xlabel!(L"w_1")
ylabel!(L"w_2")
display(p)
...
y_coarse = range(1.5*minimum(train_data.y), stop = 1.5*maximum(train_data.y), length = 30)
...
surface!(x, y, (x, y) -> pdf(Normal(w_best[1]*x+w_best[2]*x*x,0.15),y), color=:greys, fillalpha=0.1, legend=false)
wireframe!(x_coarse, y_coarse, (x, y) -> pdf(Normal(w_best[1]*x+w_best[2]*x*x,0.15),y))
xlabel!(L"x")
ylabel!(L"y")
display(p)
